# main.py - Topic 3: Online Convex Optimization (Online Logistic Regression + Regret)
...
# (T3.3) 离线基线 - 高精度投影梯度下降（备选方案，无需 CVXPY）
def compute_offline_baseline(A: np.ndarray, y: np.ndarray, R: float, max_iters: int = 3000, eta: float = 0.05):
    T, d = A.shape
    w = np.zeros(d)
...
# (T3.4) 算法2: Follow-the-Regularized-Leader (FTRL) with quadratic regularizer
...
if __name__ == "__main__":
    ...
    min_cum_loss = compute_offline_baseline(data.A, data.y, data.R)
    print(f"离线最优累计损失: {min_cum_loss:.4f}")

    # 运行 OGD (T3.4 + T3.5)
    print("运行 OGD (eta=0.1)...")
    reg_ogd, avg_ogd, _, runtime_ogd = run_ogd(data, eta=0.1, min_cum_loss=min_cum_loss)
    print(f"OGD 完成 - 总运行时间: {runtime_ogd:.2f}s")
    print(f"OGD 最终累计遗憾: {reg_ogd[-1]:.2f}, 平均遗憾: {avg_ogd[-1]:.4f}")

    # 运行 FTRL (T3.4 + T3.5)
    print("运行 FTRL (eta=0.1, lam=1.0)...")
    reg_ftrl, avg_ftrl, _, runtime_ftrl = run_ftrl(data, eta=0.1, lam=1.0, min_cum_loss=min_cum_loss)
    ...
    print("\n所有任务完成！运行 python main.py 即可复现结果和生成图片。")